...
export function DiseaseIdentificationForm() {
  ...
  const getCameraPermission = async () => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
       ...
       toast({
          variant: 'destructive',
          title: t('toast.cameraNotSupportedTitle'),
          description: t('toast.cameraNotSupportedDescription'),
        });
      return;
    }
    try {
      ...
      toast({
        variant: 'destructive',
        title: t('toast.cameraAccessDeniedTitle'),
        description: t('toast.cameraAccessDeniedDescription'),
      });
    }
  };
  ...
  async function handleSubmit() {
    if (!file) {
      toast({
        variant: 'destructive',
        title: t('toast.noImageSelectedTitle'),
        description: t('toast.noImageSelectedDescription'),
      });
      return;
    }
         ...
         {preview ? (
            <div className="space-y-4 text-center">
              <div className="relative mx-auto h-64 w-64 overflow-hidden rounded-lg">
                <Image src={preview} alt={t('form.cropPreviewAlt')} fill className="object-cover" />
              </div>
              <Button onClick={resetSelection} variant="outline">{t('form.chooseAnotherImageButton')}</Button>
            </div>
          ) : (
            <Tabs defaultValue="upload" className="w-full">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="upload" onClick={stopCamera}><FileUp className="mr-2"/> {t('form.uploadTab')}</TabsTrigger>
                <TabsTrigger value="camera" onClick={getCameraPermission}><Camera className="mr-2"/> {t('form.cameraTab')}</TabsTrigger>
              </TabsList>
              <TabsContent value="upload">
                <div className="flex flex-col items-center justify-center space-y-4 rounded-lg border-2 border-dashed border-border p-8 text-center mt-4">
                  ...
                  <p className="text-sm text-muted-foreground">{t('form.uploadText')}</p>
                  <Input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*"
                ...
                <div className="space-y-4 rounded-lg border-2 border-dashed border-border p-4 text-center mt-4">
                  {hasCameraPermission === false && (
                    <Alert variant="destructive">
                      ...
                      <AlertTitle>{t('form.cameraAccessDeniedTitle')}</AlertTitle>
                      <AlertDescription>
                        {t('form.cameraAccessDeniedDescription')}
                      </AlertDescription>
                    </Alert>
                  )}
          ...
          <Card className="bg-primary/5">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-primary">
                <ShieldCheck /> {t('results.title')}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div>
                <h3 className="font-semibold text-lg flex items-center gap-2"><AlertTriangle/> {t('results.likelyDisease')}</h3>
                <p className="mt-1 text-2xl font-bold text-primary">{aiResponse.diseaseIdentification.likelyDisease}</p>
              </div>
              <div>
                <h3 className="font-semibold text-lg flex items-center gap-2"><Percent/> {t('results.confidenceLevel')}</h3>
              ...
              <div>
                <h3 className="font-semibold text-lg flex items-center gap-2"><Lightbulb/> {t('results.possibleSolutions')}</h3>
